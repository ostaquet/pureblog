...
- The title is a link to the current language and not to the default language.
...
- Setup the deployment on Firebase Hosting.
...
- Configurable header and footer: the page header `h1` now substitutes `$site_title` from `general.site_title`, and the footer renders `© $author $year` driven by the new mandatory `general.author` field plus the build-time current year (in `publish.default_timezone`). New template placeholders: `$site_title`, `$author`, `$year` (_0030-substitute-others-tags_)
- Link support in blog posts: auto-links (`<URL>`), standard inline links (`[label](URL)`, same tab), new-tab links via a `tab:` URL prefix (rewritten at build time to `target="_blank" rel="noopener noreferrer"`), and internal post links of the form `[label](posts/{stem}.md)` (rewritten to `/{lang}/{slug}/` from any page depth). Missing internal-link targets emit a stderr warning. Documented in `docs/markdown-cheatsheet.md` (_0029-support-links_)
- Responsive image styling: post images are constrained to the article's text width via `article img { max-width: 100%; height: auto; }` so oversized images shrink to fit the layout (including on window resize) while smaller images keep their natural size. Verified end-to-end across narrow and wide viewports (_0028-format-images_)
