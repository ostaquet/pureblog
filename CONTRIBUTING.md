...
**Tip**: If you are new to the open-source community, then please read through "[How To Ask Questions The Smart Way][ext-reading]" before contributing.
...
2. If the issue is caused by you modifying the project code or some configuration, then please DO NOT report such "bugs". Pureblog is an open-source project, but that doesn't mean we will maintain other specific forks (such as yours).
...
We want to avoid chaos in the UI design and therefore do not accept requests for changes like color schemes, fontfamilies, typography, and so on. We want to avoid [feature creep][feat-creep] and focus only on the core features.
...
3. Creating a new branch from the default branch and give it a descriptive name (e.g. `add-a-new-feat` or `fix-a-bug`). When development is complete, create a [Conventional Commit][cc] with Git.
...
[latest-ver]: https://github.com/ostaquet/pureblog/releases/latest
[issues]: https://github.com/ostaquet/pureblog/issues?q=is%3Aissue
[pr]: https://github.com/ostaquet/pureblog/pulls
