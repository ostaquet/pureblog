...
Links are post-processed in `builder.transform_post_links`: a `tab:` URL prefix becomes `target="_blank" rel="noopener noreferrer"`, and `posts/{stem}.md` hrefs are rewritten to `{root}/{lang}/{slug}/`. Missing internal-link targets emit a stderr warning via `BlogBuilder.warn_missing_internal_links`.
