...
Par souci de simplicité, Pureblog utilise uniquement la date pour les articles. Vu que les flux RSS nécessitent également l'heure et le fuseau horaire, ces derniers sont configurés dans le fichier de configuration (section `publish`). Le fuseau horaire par défaut est `Europe/Brussels` (configurable via `publish.default_timezone`) et le passage à l'heure d'été est géré automatiquement.

## Description de l'article

Par défaut, les 200 premiers caractères de l'article sont utilisés comme description sur la page d'accueil, dans le flux RSS et dans les balises pour le référencement. Si vous désirez définir manuellement la description, vous pouvez la personnaliser avec le tag `excerpt` dans l'entête.
...
Pureblog calcule automatiquement un temps de lecture estimé pour chaque article (à raison de 200 mots par minute, avec un minimum d'une minute). Ce temps est affiché à côté de la date, à la fois sur la page d'accueil et sur la page de l'article. Le libellé associé (par exemple « min de lecture » en français) est configuré pour chaque langue dans `languages.reading_time_labels` du fichier de configuration.
...
Donc, vous pouvez créer les articles `042-first-post.en.md` et `042-eerst-bericht.nl.md`. Ils seront automatiquement liés à votre premier article comme traductions.
