...
The documentation for the end users lives directly in the Pureblog under `posts/`. It is maintained in French (`posts/*.fr.md`), English (`posts/*.en.md`) and Dutch (`posts/*.nl.md`) — the three languages share the same numeric prefix and **must be kept in sync**: any feature change that touches a post in one language must also update the corresponding posts in the other languages in the same commit. When adding a new user-facing post, create the `*.fr.md`, `*.en.md` and `*.nl.md` files with the same numeric prefix.

When a task adds or changes a feature, update the doc that **owns** the content rather than bloating `README.md`. Always add a `CHANGELOG.md` entry. If it is related to developement, udpate the `docs/developers.md` documentation. If it is related to a feature for the users, update the documentation under `posts/` (French, English and Dutch versions all together).
