...
- Per-task definition of done: code → tests in `src/test_*.py` → `make test` green → update `README.md` (humans) → update `CLAUDE.md` if agent-relevant → append entry in `CHANGELOG.md` referencing the task → commit → move task file from `.tasks/todo` to `.tasks/done`.
